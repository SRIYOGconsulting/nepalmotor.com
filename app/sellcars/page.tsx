    ...
    <main className="min-h-screen bg-[#090909] text-white">
      <div className="mx-auto w-full max-w-screen-2xl px-4 py-10 md:px-6 lg:px-8">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-sm text-gray-400">Browse approved used cars available for sale.</p>
        </div>
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {docs.map((car) => {
            ...
            return (
              <div key={id} className="group overflow-hidden rounded-2xl border border-white/10 bg-[#111] transition hover:-translate-y-1 hover:border-[#f4c430]/60">
                ...
                <div className="p-5">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2 text-xs text-gray-300">
                    <div className="rounded-lg border border-white/10 bg-black/20 px-3 py-2">KM: {car.kmDriven ? `${car.kmDriven}` : '-'}</div>
                    <div className="rounded-lg border border-white/10 bg-black/20 px-3 py-2">Fuel: {car.fuelType || '-'}</div>
                    <div className="rounded-lg border border-white/10 bg-black/20 px-3 py-2">Color: {car.vehicleColor || '-'}</div>
                    <div className="rounded-lg border border-white/10 bg-black/20 px-3 py-2">Trans: {car.transmission || '-'}</div>
                  </div>
