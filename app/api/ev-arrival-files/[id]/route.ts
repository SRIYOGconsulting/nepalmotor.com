...
import { findEvNewArrivalImageById, openEvNewArrivalImageDownloadStream } from '@/lib/evArrivalsGridfs';
...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const fileDoc = await findEvNewArrivalImageById(objectId);

  if (!fileDoc) {
    return Response.json({ success: false, message: 'File not found' }, { status: 404 });
  }

  const nodeStream = await openEvNewArrivalImageDownloadStream(objectId);
