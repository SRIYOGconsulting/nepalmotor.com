...
export default function CarListingGallery(props: { photoFileIds: string[]; alt: string; fallbackSrc?: string }) {
  const ids = useMemo(() => props.photoFileIds.filter(Boolean), [props.photoFileIds]);
  const [active, setActive] = useState(ids[0] || '');
  const fallbackSrc = props.fallbackSrc || '/MainLogo.png';
  ...
  const mainSrc = `/api/car-files/${active || ids[0]}`;
